...
class PlannerTrackingController extends Controller
{
    ...
    public function events(Request $request, AppointmentMailTemplateData $appointmentMailData): JsonResponse
    {
        ...
        $appointmentsQuery = $this->appointmentDetailsQuery()
        ...
        return response()->json([
            'events' => $this->appointmentEvents($appointments, $appointmentMailData),
        ]);
    }

    public function search(Request $request, AppointmentMailTemplateData $appointmentMailData): JsonResponse
    {
        abort_unless($this->canAccess($request), 403);
        ...
        $appointmentsQuery = $this->appointmentDetailsQuery()
            ->when($technicianIds->isNotEmpty(), fn ($query) => $query->whereIn('technician_id', $technicianIds))
            ->when(! empty($payload['service_id']), fn ($query) => $query->where('service_id', (int) $payload['service_id']))
            ->when(! empty($payload['date_from']), fn ($query) => $query->where('starts_at', '>=', Carbon::parse($payload['date_from'])->startOfDay()))
            ->when(! empty($payload['date_to']), fn ($query) => $query->where('starts_at', '<=', Carbon::parse($payload['date_to'])->endOfDay()));
        ...
        if ($search !== '') {
            ...
            $appointmentsQuery->where(function ($query) use ($terms): void {
                foreach ($terms as $term) {
                    $like = '%'.$term.'%';

                    $query->where(function ($query) use ($like): void {
                        $query
                            ->where('customer_first_name', 'like', $like)
        ...
        return response()->json([
            'appointments' => $this->appointmentEvents($appointments, $appointmentMailData),
            'count' => $appointments->count(),
            'limit' => $limit,
        ]);
    }

    private function appointmentDetailsQuery()
    {
        return Appointment::query()
            ->with([
                'service:id,type,name',
                'technician:id,first_name,last_name,email,address,department_code,latitude,longitude,role',
     ...
     */
    private function appointmentEvents(Collection $appointments, AppointmentMailTemplateData $appointmentMailData): Collection
    {
        ...
        return $appointments->map(function (Appointment $appointment) use ($appointmentsByTechnician, $documentsByAppointment, $appointmentMailData): array {
